...
![](Take-home_Exercise/Take-home_Ex01/images/Review%20Graph.png){width="615"}
...
Note: We are using pre-cleaned data based on the description shared by the student. Please note that some specific cleaning steps—such as the removal of rows with zero population—may not be interpreted in exactly the same way.
...
![](Take-home_Exercise/Take-home_Ex01/images/title.png){width="300" height="68"}
...
![](Take-home_Exercise/Take-home_Ex01/images/color.png)
...
![](Take-home_Exercise/Take-home_Ex01/images/axis range.png)
...
![](Take-home_Exercise/Take-home_Ex01/images/fix title.png)
...
![](Take-home_Exercise/Take-home_Ex01/images/age range fix.png)

-   The vertical axis shows every single age from 0 to 100, which makes it crowded. It's hard to read the exact population for each age because the bars are very narrow and close together.

-   Additionally, in the original data, the age values are only listed as **“90 and above”** for older groups. This makes it hard to know the exact distribution of people above 90 years old. We can’t tell whether most of them are 91 or much older, which limits the usefulness of the data when studying aging populations.

-   Also, the horizontal axis shows large numbers like 50,000 or 100,000 without explanation, which might be confusing for some viewers.

### 3.3 Content, Text, and Visual Layout

![](Take-home_Exercise/Take-home_Ex01/images/axes fix.png)

-   The axis labels are placed very close to the plot, which makes the chart feel a bit crowded and harder to read.

-   Also, while the chart gives a lot of information, it’s not easy to quickly understand the exact number of people in each group. Without clear numbers or percentages, it’s hard for viewers to get a good sense of how many people are in each age or gender category.
...
ggplot(pyramid_data_grouped, aes(x = AgeGroup, y = Population_neg, fill = Sex)) +
  ...
  theme(
    plot.title = element_text(size = 16, face = "bold", hjust = 0.5),
    plot.subtitle = element_text(size = 12, hjust = 0.5,color = "grey40"),
    axis.text.y = element_text(size = 8,  margin = margin(r = 10, l = 0)), # Add right margin
    axis.text.x = element_text(size = 10,  margin = margin(t = 5, b = 0)), # Add top margin
...
::: callout-note
**Through each part, we can see how this plot is improved.**
:::
